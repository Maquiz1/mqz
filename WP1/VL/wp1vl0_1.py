...
enrollment_path = '~/Documents/KEMR/DATA/TZ_Enrollment_Data_06_16_2025.xlsx'
...
columns_to_show = [
    'FCODE_x', 'SCRID', 'VNO_x', 'VDATE_x', 'VL', 'VLDATE', 'VL_VISIT', 'VDATE_y'
]
...
# Merge 'Eligibility Viral Load Date' and 'Eligibility Viral Load Date.1' into one column
if 'Eligibility Viral Load Date.1' in selected.columns:
    selected['Eligibility Viral Load Date'] = selected['Eligibility Viral Load Date'].combine_first(
        selected['Eligibility Viral Load Date.1']
    )
    selected = selected.drop(columns=['Eligibility Viral Load Date.1'])
